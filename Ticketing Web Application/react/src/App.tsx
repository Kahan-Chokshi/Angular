...
function App() {
  ...
  }, [dispatch,pathHistory]);
  const isAdmin: boolean = useSelector<{ auth: authObjectType }, boolean>(
    (state) => state.auth.isAdmin
  );
  const isLoggedIn: boolean = useSelector<{ auth: authObjectType }, boolean>(
    ...
    <div className="App">
      ...
      {isAdmin && (
        <Switch>
          ...
          <Route path={busesLink}>
            <BusesPage />
          </Route>
          <Route path={stopsLink}>
            <Stops />
          </Route>
