...
import {
  ...
  phoneFormReducer,
} from "../../../../reducers/phone-form-reducer";
  ...
  const onSubmitHandler = (event: React.FormEvent) => {
    ...
    fetch("http://localhost:8080/phoneAuth", {
      ...
      .then((data) => {
        ...
        authDispatch(
          authActions.setToken({ otpToken: data.verificationToken })
        );
      })
      .catch((error) => {
        console.log(error);
      ...
      <form className={"formStyle"} onSubmit={onSubmitHandler}>
        <div className={classes.formSubContainer}>
          ...
          <GradientInput
            ...
            cssClasses={[classes.numberInput]}
            onChangeHandler={onNumberChangeHandler}
            onBlurHandler={onBlurHandler}
          />
        </div>
        {!state.isPhoneNumberValid && (
          <p className={"error"}>{state.phoneNumberErrorMessage}</p>
        )}
