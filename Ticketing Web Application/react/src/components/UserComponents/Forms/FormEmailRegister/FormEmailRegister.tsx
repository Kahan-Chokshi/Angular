...
} from "../../../../reducers/email-form-reducer";
...
const FormEmailRegister: React.FC<{}> = () => {
  ...
  const { isEmailValid, isPasswordValid, isConfirmPasswordValid, isFormValid } =
    state;
  ...
  return (
    <div className={"formContainer"}>
      ...
      <form className={"formStyle"} onSubmit={onsubmitHandler}>
        ...
        {!state.isEmailValid && state.emailErrorMessage.length > 0 && (
        ...
        {!state.isPasswordValid && state.passwordErrorMessage.length > 0 && (
        ...
        <NormalGradientButton
          text={"Register"}
          buttonColor={color}
          cssClassesOnContainer={[classes.btn]}
          disabled={!state.isFormValid}
          type={"submit"}
        />
        <Link className={"link"} to={emailLoginLink}>
          Already Have an Account? Click Here.
        </Link>
        <Link className={"link"} to={emailPasswordResetLink}>
          Forgot Password?
        </Link>
        <Link className={"link"} to={loginLink}>
          Get in With Other Methods
        </Link>
      </form>
    </div>
  );
