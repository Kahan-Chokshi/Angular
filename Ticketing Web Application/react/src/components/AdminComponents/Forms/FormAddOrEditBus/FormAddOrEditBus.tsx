...
const FormAddOrEditBus: React.FC<{
  ...
  const selectedBusId = useSelector(
    (state: RootState) => state.busesList.selectedBusId
  );
  ...
  const onSaveHandler = () => {
    if (props.mode === "Add") {
      saveBus(
        "http://localhost:8080/admin/addNewBus",
        {
    ...
    if (props.mode === "Edit") {
      saveBus(
        "http://localhost:8080/admin/editBus",
        {
          _id: selectedBusId,
          busNumber: busNumberInputRef.current.value,
          stops: stops,
  ...
  const onBusNumberChangeHandler = (
  ...
  return (
    <GrayCard cssClasses={[classes.overAllContainer]}>
      ...
      <GradientInput
        ...
        onChangeHandler={onBusNumberChangeHandler}
          ...
          {stops.map((stop) => {
            return (
              <p
                ...
                onClick={() => onStopsClickHandler(stop._id)}
              >
                {stop.name}
