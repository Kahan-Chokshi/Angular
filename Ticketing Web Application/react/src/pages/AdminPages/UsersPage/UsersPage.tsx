...
import HistoryResults, {
  ticketHistory,
} from "../../../components/HistoryResults/HistoryResults";
...
const UsersPage: React.FC<{}> = () => {
  ...
  const isAddOpen = useSelector(
    (state: RootState) => state.usersList.isAddOpen
  );
  const isHistoryOpen = useSelector(
    (state: RootState) => state.usersList.isHistoryOpen
  );
  const isResetOpen = useSelector(
    (state: RootState) => state.usersList.isResetOpen
  );
  const isDeleteOpen = useSelector(
    (state: RootState) => state.usersList.isDeleteOpen
  );
  const selectedUserId = useSelector(
    (state: RootState) => state.usersList.selectedUserId
  );
  useEffect(() => {
  ...
  return (
    <Fragment>
      ...
      <BackDrop
        visibility={`${isHistoryOpen ? "show" : "hide"}`}
        onClick={onCloseHandler}
      >
        <HistoryResults
          history={history}
      ...
      <BackDrop
        visibility={`${isResetOpen ? "show" : "hide"}`}
        onClick={onCloseHandler}
      >
